...
# Prepare files for cfp process
      ...
      if [  "$FHMAX_WAV_CUR" -le 72 ]; then 
        rtofsfile1="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f024_prog.nc"
        rtofsfile2="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f048_prog.nc"
        rtofsfile3="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f072_prog.nc"
        if [ ! -f $rtofsfile1 ] || [ ! -f $rtofsfile2 ] || [ ! -f $rtofsfile3 ]; then 
           #Needed current files are not available, so use RTOFS from previous day 
           export RPDY=$($NDATE -24 ${RPDY}00 | cut -c1-8)
        fi 
      else
        rtofsfile1="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f096_prog.nc"
        rtofsfile2="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f120_prog.nc"
        rtofsfile3="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f144_prog.nc"
        rtofsfile4="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f168_prog.nc"
        rtofsfile5="${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_f192_prog.nc"
      ...
      do
        ...
        curfile1h=${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_${fext}${fh3_rtofs}_prog.nc
        curfile3h=${COM_RTOFS}/rtofs.${RPDY}/rtofs_glo_2ds_${fext}${fh3_rtofs}_prog.nc
